...
const adminMenus: { label: string; screens: Screen[]; target: Screen }[] = [
  ...
  { label: "운영과 관리", screens: ["adm-operators"], target: "adm-operators" },
...
export function AdminLayout({
  ...
  return (
    <div className="flex min-h-screen" style={{ background: C.bg, fontFamily: "'Noto Sans KR', system-ui, sans-serif" }}>
      <aside
        style={{ width: 220, background: "#1e293b", color: "#e2e8f0", flexShrink: 0, position: "sticky", top: 0, height: "100vh", overflowY: "auto" }}
        className="flex flex-col"
      >
        <button
          onClick={() => navigate("adm-dashboard")}
          className="px-5 py-5 font-bold text-base text-left border-b border-slate-700"
        >
          팝콘PC 관리자
        </button>
        ...
        <button
          ...
          className="mx-3 mb-3 px-4 py-2.5 rounded-md text-left text-sm font-semibold"
          style={{ background: "rgba(255,255,255,0.08)", color: "#cbd5e1" }}
        >
          DEV 허브
        </button>
        <div className="px-5 py-4 text-sm border-t border-slate-700 text-slate-400">관리자 모드</div>
      </aside>
